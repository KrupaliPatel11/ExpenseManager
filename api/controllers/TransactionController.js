...
module.exports = {
    ...
    // Add Transaction
    add: async (req, res) => {
        const { accId, amount, status, description } = req.body
    ...
    update: async (req, res) => {
        ...
        if (owner || member) {
            ...
            if(trans.isDeleted == true) {
                return res.send({Error : "Transaction is deleted"})
            }
            else {
    ...
    delete: async (req, res) => {
        ...
        if (trans.isDeleted == true) {
            ...
            .set({ isDeleted: true, deletedAt: new Date().toLocaleString(), deletedBy : userId })
            res.send({
    ...
    list: async (req, res) => {
        ...
        if (account.length > 0 || member.length > 0) {
            const trans = await Transaction.find({ accId: accId, isDeleted: false }).sort('transactionDate DESC')
            console.log(trans);
            return res.send({
